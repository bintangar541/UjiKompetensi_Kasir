    ...
    public function index()
{
    ...
    // Query untuk produk dan jumlah penjualan
    $productShell = detail_sales::with('product')
        ->selectRaw('product_id, SUM(amount) as total_amount')
        ->groupBy('product_id')
        ->get();

    // Periksa apakah produk ada sebelum mencoba mengakses properti 'name'
    $labelspieChart = $productShell->map(fn($item) => $item->product ? $item->product->name . ' : ' . $item->total_amount : 'Produk Tidak Ditemukan')->toArray();
